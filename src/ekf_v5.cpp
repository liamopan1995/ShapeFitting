/// based on ekf_v3 , with a new io that save both imu and utm into  one single file in the order of time stamp
/// each row cotains a tag , where 1 stands for utm data, 0 for imu.
/// Oct 10th.

///  DATA format :   
///  UTM row : 1, time stamp, x,           y,                       v,        theta(rad)
///  IMU row : 0, time stamp, acceleration a, agular velocity wphi, 0,        0  


///  NEW IN THIS ver:
///  use a revised state model , seperate state v into v_x and v_y , 
///  new state vector [ s_x, s_y,_s_phi, v_x, v_y]
...
public:
    ...
    std::vector< Eigen::Matrix<double, 1,6>> utm_ctrl; // sore both utm and imu in the order of time stamp
...
void EKF::Predict(const sensor_msgs::Imu::ConstPtr &msg_in) {
    ...
    Eigen::Matrix<double, 1,6> utm_ctrl_row;
    utm_ctrl_row << 0, msg_in->header.stamp.toSec()-initial_time_,a,wphi,0, 0;
    utm_ctrl.push_back(utm_ctrl_row);
    //std::cout<<"utm_ctrl\n"<< utm_ctrl_row(0) <<utm_ctrl_row(1)<<"\t"<<utm_ctrl_row(2)<<"\t"<<utm_ctrl_row(3)<<utm_ctrl_row(4)<<utm_ctrl_row(5)<<std::endl;

    /// Update the states  model is correct as dt is significantlly small
    ...
    F(0,2) = - v_ * sin(sphi_)*dt_;
    F(0,3) =     cos(sphi_)*dt_;
    F(1,2) =   v_ *cos(sphi_)*dt_;
    F(1,3) =     sin(sphi_)*dt_;
...
void EKF::Update(const sensor_msgs::NavSatFix::ConstPtr& msg_in){
    ...
    /// before modification  oct 10:
    //float measurement_theta = atan2(observation_2d[0]-utm_last[0], observation_2d[1]-utm_last[1]);
    float measurement_theta = atan2(observation_2d[1]-utm_last[1], observation_2d[0]-utm_last[0]);
    ...
    Eigen::Matrix<double, 1,6> utm_ctrl_row;
    utm_ctrl_row << 1, msg_in->header.stamp.toSec()-initial_time_,observation_3d[0],observation_3d[1],
    observation_3d[2], measurement_theta;

    utm_ctrl.push_back(utm_ctrl_row);
    //std::cout<<"utm_ctrl\n"<< utm_ctrl_row(0) <<utm_ctrl_row(1)<<"\t"<<utm_ctrl_row(2)<<"\t"<<utm_ctrl_row(3)
    //<<"\t"<<utm_ctrl_row(4)<<utm_ctrl_row(5)<<std::endl;
    ...
    ///  Align UTM format to IMU foramt of representation of velocity and ( theta )
...
int main(int argc, char **argv) {
    ...
    Eigen::Vector2f process_covariance(0.1, 0.2); // Vector with diagonal values
    Eigen::Vector3f observation_covariance(0, 0, 3); // Vector with diagonal values
    ekf.Q_ = process_covariance.asDiagonal();
    // ekf.R_ = Observation_cov::Zero();
    ekf.R_ = observation_covariance.asDiagonal();
    ekf.P_ = State_cov::Identity() * 1e4;
    // ekf.P_(2,2) = ;
    ...
    std::string fullPath = std::string(homeDir) + "/slam_in_autonomous_driving/data/ch3/velodyne_bag_txt/state_imu_2d_ekf_v4.txt";
    ...
    // std::ofstream utm_file( std::string(homeDir) +"/slam_in_autonomous_driving/data/ch3/velodyne_bag_txt/utm_ctrl.txt");
    // if (utm_file.is_open()) {
    //     for (const auto & row : ekf.utm_ctrl) {
    //         /// if it is a utm row:
    //         if(row(0)==1) {
    ...
    //     utm_file.close();
    // } else {
    //     std::cerr << "Error: Unable to open utm_ctrl file for writing." << std::endl;
    // }
