...
///  IMU row : 0, time stamp,  ax,ay,az,  wphix, wphiy, wphiz        ori_0,   ori_1, ori_2, ori_3,
...
public:
    ...
    std::vector< Eigen::Matrix<double, 1,12>> utm_ctrl; // sore both utm and imu in the order of time stamp
...
void EKF::Predict(const sensor_msgs::Imu::ConstPtr &msg_in) {
    ...
    Eigen::Matrix<double, 1,12> utm_ctrl_row;
    utm_ctrl_row << 0,
    msg_in->header.stamp.toSec()-initial_time_,
...
void EKF::Update(const sensor_msgs::NavSatFix::ConstPtr& msg_in){
    ...
    observation_3d[2], measurement_theta,0,0,0,0,0,0;
...
int main(int argc, char **argv) {
    ...
    std::ofstream utm_file( std::string(homeDir) +"/slam_in_autonomous_driving/data/ch3/velodyne_bag_txt/utm_ctrl_v2.txt");
    if (utm_file.is_open()) {
