...
Circle CircleFitByLeastSquare(Eigen::Matrix<float, Eigen::Dynamic, 3> P)
{
    Circle circle;
    MatrixXf A (P.rows(),3);// A = P , but with P's all element in its third colum set to 1  
    VectorXf b(P.rows(),3);// = P first colum's suqrare + P second colum's suqrare

    // assign values from P , without using for loop
    // for (size_t i = 0; i < x.size(); ++i) {
    //     A(i, 0) = x[i];
    //     A(i, 1) = y[i];
    //     A(i, 2) = 1.0;
